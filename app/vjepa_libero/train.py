...
from app.vjepa_libero.libero import init_libero_data
...
def main(args, resume_preempt=False):
    ...
    input_length = cfgs_data.get("input_length", 16)
    train_episodes = cfgs_data.get("train_episodes", 1200)
    action_shift = cfgs_data.get("action_shift", 0)
    ...
    task_suite = cfgs_data.get("task_suite", "libero_100")
    use_task_conditioning = cfgs_data.get("use_task_conditioning", True)
    include_language_instructions = cfgs_data.get("include_language_instructions", True)
    ...
    encoder, predictor = init_video_model(
        ...
        use_extrinsics=False, # TODO: currently hand fixed to False
        use_sdpa=use_sdpa,
        use_silu=use_silu,
    ...
    # -- init LIBERO data-loaders/samplers
    (unsupervised_loader, unsupervised_sampler) = init_libero_data(
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr in range(ipe):
            ...
            def load_clips():
                # Handle LIBERO dataset format which includes language instructions
                clips = sample['image'].to(device, non_blocking=True).permute(0, 2, 1, 3, 4)  # [B C T H W]
                actions = sample['actions'].to(device, dtype=torch.float, non_blocking=True)  # [B T-1 7]
                states = sample['state'].to(device, dtype=torch.float, non_blocking=True)  # [B T 7]
                # extrinsics = sample[3].to(device, dtype=torch.float, non_blocking=True)  # [B T 7]
                # language_instructions = sample[5]  # List of language instructions
                return (clips, actions, states, None, None)

            clips, actions, states, extrinsics, language_instructions = load_clips()
            ...
            def train_step():
                ...
                def forward_predictions(z):
                    ...
                    _z, _a, _s= z[:, :-tokens_per_frame], actions, states[:, :-1]
