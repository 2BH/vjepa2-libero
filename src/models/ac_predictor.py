...
class VisionLRNNPredictorAC(nn.Module):
    def __init__(
        self,
        img_size=(224, 224),
        patch_size=16,
...
class VisionTransformerPredictorAC(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.predictor_embed = nn.Linear(embed_dim, predictor_embed_dim, bias=True)
        self.action_encoder = nn.Linear(action_embed_dim, predictor_embed_dim, bias=True)
        self.state_encoder = nn.Linear(action_embed_dim if state_embed_dim is None else state_embed_dim, predictor_embed_dim, bias=True)
        self.extrinsics_encoder = nn.Linear(action_embed_dim - 1, predictor_embed_dim, bias=True)
